"""
This script is used to generate Figures B.2(left) and B.3(left), providing illustrations
...
PD scenario and the local pairwise group-level update rule. This figure uses data included 
...
file1 = pairwise_folder + "/Simulation_Outputs/Rho1xPDgamma1p5_Local.txt"
file2 = pairwise_folder + "/Simulation_Outputs/AvgGPDgamma1p5_Local.txt"
file3 = pairwise_folder + "/Simulation_Outputs/PeakPDgamma1p5_Local.txt"
file4 = pairwise_folder + "/Simulation_Outputs/MeanCoopPDEgamma1p5_Local.txt"
...
def lambda_star_PD(alpha,beta,gamma,P,s, theta):
	num = pi_diff(alpha,beta) * theta
	denom = rho(1,0,alpha,gamma,P,s) - rho(0,1,alpha,gamma,P,s)
	return num / denom
...
local pairwise group-level victory probability and game-theoretic parameters given by
...
threshold_lamb = lambda_star_PD(alpha,beta,gamma,P,s,theta)
...
plt.savefig(pairwise_folder + "/Figures/PD_rho1x_plot_Local.png")
...
plt.savefig(pairwise_folder + "/Figures/avgx_plot_Local.png")
...
plt.annotate(r"$G(x^*)$", xy = (60.,1.795), fontsize =18.)
plt.annotate(r"$G(\overline{x})$", xy = (60.,1.545), fontsize =18.)

plt.axis([0.,120.,0.,2.5])

plt.tight_layout()

plt.savefig(pairwise_folder + "/Figures/avgG_plot_Local.png")
