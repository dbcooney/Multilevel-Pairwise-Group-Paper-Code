"""
This script is used to generate Figures 6.7 and 6.8, providing illustrations
...
HD scenario and the Fermi update rule. This figure uses data included 
...
file1 = pairwise_folder + "/Simulation_Outputs/Rho1xHDgamma3p5P1.txt"
file2 = pairwise_folder + "/Simulation_Outputs/AvgGHDgamma3p5P1.txt"
file3 = pairwise_folder + "/Simulation_Outputs/PeakHDgamma3p5P1.txt"
file4 = pairwise_folder + "/Simulation_Outputs/MeanCoopHDgamma3p5P1.txt"
...
def lambda_star_HD(alpha,beta,gamma,s, theta):
	x_eq = beta / (-alpha)
	num = pi_diff(alpha,beta) * theta
	denom = rho(1,x_eq,alpha,gamma,s) - rho(x_eq,1,alpha,gamma,s)
	return num / denom
...
theta = 1.
	...
	"""
Reading in data from simulation outputs for the case of a multilevel HD scenario with the
Fermi group-level victory probability and game-theoretic parameters given by
\gamma = 3.5, \alpha = -2, and \beta = 1.
"""
...
plt.axvline(x = lambda_star_HD(alpha,beta,gamma,s,theta), lw = 5., ls = '-.', alpha = 0.8, color = 'k')
...
plt.annotate(r"$\lambda^*_{HD}$", xy = (5.,0.1), fontsize = 20.)



plt.tight_layout()

plt.savefig(pairwise_folder + "/Figures/HD_rhox1_plot.png")
...
plt.axhline(y = gamma / (-2. * alpha), lw = 5., ls = '-.', color = 'k', alpha = 0.8)
plt.axhline(y = 0.75, lw = 5., ls = '-.', color = 'k', alpha = 0.8)
plt.axhline(y = 0.5, lw = 5., ls = '-.', color = 'k', alpha = 0.8)

plt.axvline(x = lambda_star_HD(alpha,beta,gamma,s,theta), lw = 5., ls = '-.', alpha = 0.8, color = 'k')

plt.annotate(r"$x^* = 0.875$", xy = (42.,.895), fontsize =18.)
plt.annotate(r"$\overline{x} = 0.75$", xy = (42.,.775), fontsize =18.)
plt.annotate(r"$x_{\mathrm{eq}} = 0.5$", xy = (42.,.525), fontsize =18.)

plt.annotate(r"$\lambda^*_{HD}$", xy = (5.,0.3), fontsize = 18.)

plt.legend(loc = "lower right", fontsize = 20.)

plt.tight_layout()

plt.savefig(pairwise_folder + "/Figures/HD_avgx_plot.png")
